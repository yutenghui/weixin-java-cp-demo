...
public class WeChatServiceImpl implements WeChatService {
    ...
    private static final String corpid2 = "ww4832047240e12766";
    private static final String corpsecret2 = "uMcw_y0zIjc5UyEwehigB7oQB9jn1B8o2dOfxnQluUc";
    ...
    public String getAccessToken(Integer agentid){
        String accessToken = "";
        Object cacheObject = redisUtil.getCacheObject(QiyeWXConstant.WX_ACCESS_TOKEN+"_"+agentid);
        if (cacheObject != null){
            accessToken = String.valueOf(cacheObject);
        }else {
            log.info("请求access_token");
            Map<String,Object> params = new HashMap<>();
            params.put("corpid", corpid);
            params.put("corpsecret", corpsecret2);
            String result = OkHttpUtils.get(accessTokenUrl, null, params);
            if (StringUtils.isEmpty(result)){
                log.error("获取access_token失败");
            }
            JSONObject jsonObject = JSONObject.parseObject(result);
            accessToken = jsonObject.getString("access_token");
            redisUtil.setCacheObject(QiyeWXConstant.WX_ACCESS_TOKEN+"_"+agentid,accessToken,30, TimeUnit.MINUTES);
        }
        return accessToken;
    }
